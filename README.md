...
- **Privacy-aware** — automatically skips clipboard items marked sensitive by password managers and similar apps (`org.nspasteboard.ConcealedType` / `TransientType` / `AutoGeneratedType`), so they never enter history.
...
The "Plain" chip in the panel footer toggles plain-text-only pasting for text items; hold `Shift` while activating to invert it for a single paste.
