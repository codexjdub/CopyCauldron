...
- **Drag into the panel** — drop text selections, images, or files from any other app onto the panel to add them to history without going through the pasteboard. The panel border accent-tints while a drop is incoming; dropped items enter regular history (press `p` to pin if you want to keep them).
...
- **Right-click actions** — Reveal in Finder, Open, Copy path as text (files); Save as…, Open in Preview, Paste OCR text (images); Open in browser (URLs). **Share…** on any row opens the macOS share menu (AirDrop, Messages, Mail, Notes, Reminders, custom share extensions).
